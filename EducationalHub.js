...
const EducationalHub = () => {
  ...
  useEffect(() => {
    if (quizStarted) {
      // Ensure allQuestions is not empty
      if (allQuestions.length > 0) {
        ...
        setQuizStarted(false); // Exit quiz mode if no questions are available
      }
    }
  ...
  const handleAnswer = (answer) => {
    ...
    setTimeout(() => {
      ...
      if (currentQuestionIndex < selectedQuestions.length - 1) {
        ...
        Alert.alert(
          'Quiz Completed!',
          `Your score: ${score + (isCorrect ? 1 : 0)}/${selectedQuestions.length}`,
          [{ text: 'OK', onPress: () => setQuizStarted(false) }]
        );
        ...
        <>
          ...
          <View style={styles.tipCard}>
            ...
            <Text style={styles.tipSubtitle}>
              Learn something new every day and make a difference!
            </Text>
          ...
          {/* Modernized "Take a Quiz" Segment */}
          <View style={styles.quizCard}>
            <MaterialIcons
              name="quiz" // Use a quiz-related icon
              size={60} // Increased icon size
              color="#4CAF50" // Green theme color
            ...
            <Text style={styles.quizSubtitle}>
              Take a quick quiz and earn points while learning!
            </Text>
